...
const BookCard = () => {
  ...
  const filterData = book.filter((item) => item.title.toLowerCase().includes(input.toLowerCase()));

  return (
    <>
      <input type="text" placeholder='search the card' className='p-3 border-2 border-black rounded-2xl mt-5' onChange={(e) => setInput(e.target.value)} value={input} />
                ...
                <li className="pt-3 pb-0 sm:pt-4">
                  <div className="flex-col gap-20 items-center">
                    ...
                    <div className="inline-flex items-center text-base font-semibold text-gray-900 mt-5 gap-4">
                      <button className="bg-green-600 text-white rounded-full hover:bg-green-100 hover:text-green-700 border hover:border-green-700 py-1 px-3"  onClick={()=>handleUpdate(details.title)}>
                        Update
                      </button>
                      <button
