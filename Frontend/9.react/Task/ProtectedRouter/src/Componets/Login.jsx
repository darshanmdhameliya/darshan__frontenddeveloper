...
const Login = () => {
    ...
    const handlesubmit = (e) => {       
        ...
        if (formData.username === user.username && formData.password === user.password) {
            ...
            alert('Invalid User Plese  SignUp')
        }
    }
                ...
                <div className='container mx-auto flex flex-col justify-center items-start pl-40 gap-6 h-full' >
                    <div className='flex flex-col justify-center items-start gap-6 border-2 border-yellow-200 p-10 rounded-3xl '>
                        <label htmlFor="title" className='flex gap-5 items-center'>
                            ...
                            <input type="text" placeholder='enter a username' value={formData.username} onChange={(e) => setformData((prev) => ({ ...prev, username: e.target.value }))} className='p-2' />
                        </label>

                        <label htmlFor="author" className='flex gap-5 items-center'>
                            <p className='text-white'> Password </p>
                            <input type="password" placeholder='Add a password  ' value={formData.password} onChange={(e) => setformData((prev) => ({ ...prev, password: e.target.value }))} className='p-2' />
                        </label>
